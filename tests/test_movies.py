...
class TestMovies:
    ...
    # ---------------------------------------
    def test_filter_movies_title_year_range_sort_limit(self, monkeypatch):
        connection, cursor = make_mocks()
        monkeypatch.setattr(repo, "Db", type("_D", (), {"connect": staticmethod(lambda: connection)}))

        rows = [
            {"id": 1, "title": "Matrix Reloaded", "year": 2003, "genre": "Sci-Fi",
             "director": "Wachowski", "rating": 7.2, "language": "en", "created_at": "2024-01-01 00:00:00"}
        ]
    ...
    # -----------------------------------------------------
    def test_filter_movies_genre_list_and_director(self, monkeypatch):
        connection, cursor = make_mocks()
        monkeypatch.setattr(repo, "Db", type("_D", (), {"connect": staticmethod(lambda: connection)}))
    ...
    # ------------------------------------------
    def test_filter_movies_with_actor_adds_joins(self, monkeypatch):
        connection, cursor = make_mocks()
        monkeypatch.setattr(repo, "Db", type("_D", (), {"connect": staticmethod(lambda: connection)}))

        cursor.fetchall.return_value = [{
            "id": 10, "title": "Speed", "year": 1994, "genre": "Action",
            "director": "De Bont", "rating": 7.3, "language": "en", "created_at": "2024-01-01"
        }]

        result = repo.filter_movies(filters={"actor": "Keanu"}, db="akali_movies", limit=3, offset=0)
    ...
    # ------------------------
    def test_get_movie_by_id(self, monkeypatch):
        connection, cursor = make_mocks()
        ...
        row = {"id": 7, "title": "Interstellar", "year": 2014, "genre": "Sci-Fi",
               "director": "Christopher Nolan", "rating": 8.6, "language": "en", "created_at": "2024-01-01"}
    ...
    # -------------------------
    def test_search_movies_text(self, monkeypatch):
        connection, cursor = make_mocks()
        ...
        rows = [{"id": 1, "title": "E.T.", "year": 1982, "genre": "Family",
                 "director": "Steven Spielberg", "rating": 7.8, "language": "en", "created_at": "2024-01-01"}]
        cursor.fetchall.return_value = rows
